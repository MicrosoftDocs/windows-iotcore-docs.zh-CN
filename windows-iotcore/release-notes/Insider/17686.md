...
---
# <a name="release-notes-for-build-17686"></a>发行说明生成 17686
_生成号 17686。 2018 年 5 月。_
...
* * 你现在应能够像往常一样将调试器连接
* 无法在 UWP 应用中使用 Node.js 和 Python。 在这些语言中的较旧示例可能无法再正常工作。 Microsoft 正在考虑用于将来的版本的替代方法。 
* "你好，小娜"MBM 64，QDB 上将不响应 Cortana SDK。 
* CortanaSDK 不会返回 web 搜索结果。 
* 安装智能显示应用时出错。 
* MiraCast 无法连接到硬件保护装置 （QDB-> 硬件保护装置)。
* DMDashboard 引发异常在尝试读取后 Azure 注册的设备信息。
* OpenSSH 不使用 IoTCore 正常工作。
* UWF 和操作系统更新上，不要把齿轮。

...
---
# <a name="release-notes-for-build-17686"></a>版本17686的发行说明
_内部版本号17686。可能为2018。_
...
**  现在应该能够正常连接调试程序
* 无法在 UWP 应用中使用 node.js 和 Python。 这些语言中较旧的示例可能无法再正常工作。 Microsoft 正在考虑将替代项用于将来的版本。 
* Cortana SDK 将不会响应 MBM 64, QDB 上的 "你好 Cortana"。 
* CortanaSDK 不会返回 web 搜索结果。 
* 安装智能显示应用时出错。 
* MiraCast 无法连接到转换器 (QDB-> 转换器)。
* DMDashboard 在 Azure 注册后尝试读取设备信息时引发异常。
* OpenSSH 在 IoTCore 中不能正常工作。
* 不会在 UWF 和 OS 更新上旋转齿轮。

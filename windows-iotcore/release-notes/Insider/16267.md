...
---
# <a name="release-notes-for-build-16267"></a>发行说明生成 16267

_生成号 16267。 2017 年 8 月_
...
* 一般性的 bug 修复 
* 由于 Joule Intel EOL （生命），将很快停止 Joule 的 FFUs。 没有 Joule FFU 将针对 RS3 RTM 发布。 客户应确定一种替代方法使用的另一个平台支持 Soc-[的详细信息](../../learn-about-hardware/SoCsAndCustomBoards.md)
* IOT_WEBB_EXTN 功能已被重构以删除载入功能。 现在已可用作 IOT_ONBOARDING_APP 载入功能。
利用此更新，将删除载入功能并使用载入功能的用户应重新刷新其设备再次获取此功能。
* 多语言包现已推出。 可以创建 FFUs 支持多种语言支持，请参阅[MultiLangSample](https://github.com/ms-iot/iot-adk-addonkit/tree/16299/Source-arm/Products/MultiLangSample)并[SingleLangSample](https://github.com/ms-iot/iot-adk-addonkit/tree/16299/Source-arm/Products/SingleLangSample)有关详细信息。
* 你可以使用 IoTSettings.exe 的系统语言。 提供了详细信息[此处](https://developer.microsoft.com/en-us/windows/iot/docs/cortanaoniotcore)语言配置部分中。

## <a name="additional-information"></a>其他信息
* 使用我们的龙第板映像的 BSP 版本是 2112.0.0.0 


## <a name="known-issues-in-this-build"></a>此版本中的已知问题：
* 应用商店发布应用程序不能调用通过 Windows.System.ProcessLauncher exe，除非 Microsoft 签名。
* 不使用 visual Studio 调试适用于 JavaScript 应用。 应用可以部署和启动不从 Visual Studio 调试器。
应用还可以从 Visual Studio 部署和启动从设备门户或 iotstartup.exe。
* 网络连接故障中 MinnowBoard 最大值 (MBM) 固件版本 0.93。 （固件版本 0.94 中修复）。 
* 从 Visual Studio 的 F5 驱动程序部署不适用于 IoT Core。 
* 映像创建可能由于 InstallOemCerts.cmd 命令错误而失败。
* 后台任务不显示在应用程序管理器视图中 Windows Device Portal (WDP)。
...
Raspberry Pi 3 具备载入必须禁用使用不同的硬件保护装置禁用到板载蓝牙，打开蓝牙`telnet/ssh session`并运行：
...
#### <a name="graphics-support-limited"></a>图形支持有限 
若要启用硬件加速的图形驱动程序不包括 Joule 映像中。 系统将使用软件呈现，这会影响性能和 CPU 使用率。 

### <a name="for-minnowboard"></a>有关 MinnowBoard
#### <a name="minnowboard-max-boot-and-firmware-update"></a>Minnowboard Max 启动和固件更新 
除非固件版本.092，否则将不会启动 MinnowBoard 最大或更高版本。 最小建议的固件的版本是"MinnowBoard 最大 0.92 32-Bit"。 可以从下载固件更新  
...
以下代码行可能会崩溃： `BackgroundMediaPlayer.MessageReceivedFromForeground += OnMessageReceivedFromForeground;`。 若要防止在发生崩溃，添加此代码，以便首先执行 `var player = BackgroundMediaPlayer.Current;`

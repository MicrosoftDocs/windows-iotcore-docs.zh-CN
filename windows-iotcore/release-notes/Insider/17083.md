...
---
# <a name="release-notes-for-build-17083"></a>发行说明生成 17083
_生成号 17083。 2018 年 2 月_
...
* 一般性的 bug 修复 
* 启用 Flash IOTUAP 图像中的模式。


## <a name="additional-information"></a>其他信息
* 使用我们的龙第板映像的 BSP 版本是 2116.0.0.0 
...
*   麦克风不起作用。
*   鼠标指针不可见的龙板映像上。
*   无法在无外设模式中部署应用。 设备不在无外设模式下起作用。
*   F5 主的前台 UWP 应用的应用程序部署不会在首次尝试。 第二次尝试应成功。
*   IoT 核心版默认应用将替换为 UWP 启动应用。
...
除非固件版本.092，否则将不会启动 MinnowBoard 最大或更高版本。 最小建议的固件的版本是"MinnowBoard 最大 0.92 64-Bit"。 可以从下载固件更新[此处](https://firmware.intel.com/projects/minnowboard-max)。
...
例如： checknetisolation loopbackexempt-a-n = IoTOnboardingTask uwp_1w720vyc4ccym 重新启动。 
...
* [添加到图像文件和注册表设置](https://docs.microsoft.com/windows-hardware/manufacture/iot/add-a-registry-setting-to-an-image)

### <a name="starting-the-ftp-server"></a>启动 FTP 服务器 
FTP 服务器无法再运行默认情况下在启动时 

若要运行一次： 
* 使用 SSH\PS，然后运行此命令启动 FTP 登录名：  
  ```
  start ftpd.exe 
  ```

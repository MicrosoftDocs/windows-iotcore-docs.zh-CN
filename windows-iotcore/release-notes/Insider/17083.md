...
---
# <a name="release-notes-for-build-17083"></a>版本17083的发行说明
_内部版本号17083。2018年2月_
...
*   麦克风不起作用。
*   鼠标指针在龙板图像上不可见。
*   无法在无外设模式下部署应用程序。 设备在无外设模式下无法正常工作。
*   第一次尝试时, 应用程序头前景 UWP 应用程序的应用程序部署将无法工作。 第二次尝试应成功。
*   UWP 启动应用将替换为 IoT Core 默认应用。
...
除非固件是 .092 或更高版本，否则 MinnowBoard Max 不会启动。 最小建议固件版本为 "MinnowBoard MAX 0.92 64-Bit"。 固件更新可从[此处](https://firmware.intel.com/projects/minnowboard-max)下载。
...
例如: checknetisolation loopbackexempt-n = IoTOnboardingTask-uwp_1w720vyc4ccym Reboot。 
...
* [向映像中添加文件和注册表设置](https://docs.microsoft.com/windows-hardware/manufacture/iot/add-a-registry-setting-to-an-image)

### <a name="starting-the-ftp-server"></a>启动 FTP 服务器 
FTP 服务器不再在启动时默认运行 

若要运行一次： 
* 请使用 SSH\PS 登录，然后运行以下命令来启动 FTP：  
  ```
  start ftpd.exe 
  ```
...
* schtasks/create/tn "IoTFTPD"/tr al.exe 

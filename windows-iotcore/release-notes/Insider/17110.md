...
---
# <a name="release-notes-for-build-17110"></a>版本17110的发行说明
_内部版本号17110。2018年3月。_
...
* 通过 NOOBS 安装的设备不能运行 bcdedit 工具来启用内核调试程序。 可以通过以下解决方法实现此目的:
* 在电脑上装载 SD 卡
* 查找 EFIESP 驱动器分区号 with diskpart 或磁盘管理 (假设它是 "M:")
* 运行命令 "bcdedit/store M:\EFI\Microsoft\boot\bcd/set {default} debug yes"
* 卸载 SD 卡。
* 你现在应该能够照常连接调试程序
* 第一次尝试时, 应用程序头前景 UWP 应用程序的应用程序部署将无法工作。 第二次尝试应成功。
* 无法禁用 UWF。
* RPi3 不在无外设模式下启动。
* 无法使用 NOOBS 安装映像。
* 将生成安装到 eMMC 时, Hummingboard 无法启动。
...
* 调整音量, 使两者都处于 40-70% 范围内。 (请确保麦克风不是 0.0)  
...
#### <a name="dragonboard-410c-shutdown"></a>DragonBoard 410c 关闭
...
DragonBoard BSP 包含用于耳机插孔和麦克风插孔的驱动程序, 但它不具有这两个板上的插孔。  

#### <a name="dragonboard-spi-runs-at-48mhz"></a>DragonBoard SPI 以 4.8Mhz 的速度运行
DragonBoard 上的 SPI 将忽略请求速度并始终在 4.8 Mhz 运行。  
...
除非固件是 .092 或更高版本，否则 MinnowBoard Max 不会启动。 推荐的最低固件版本是“MinnowBoard MAX 0.92 32 位”。 可以从此[页面](http://go.microsoft.com/fwlink/?LinkId=708613)下载固件更新。
...
解决方案：可以通过 SSH 或 Powershell 连接到设备, 并使用工具 devcon 删除远程传感器驱动程序, 方法是通过键入以下内容来删除该驱动程序:
...
某些 USB 键盘和鼠标可能无法在 IoT 核心版上工作。 使用其他键盘或鼠标。 可在[此处](https://docs.microsoft.com/windows/iot-core/learn-about-hardware/hardwarecompatlist)的文档中找到经过验证的外围设备的列表。
...
2) 也可根据需要通过启动脚本或自定义运行时配置包（在创建映像过程中包括进来）将其添加到注册表中。 有关更多详细信息, 请参阅[Windows 10 IoT 核心制造指南](https://docs.microsoft.com/windows-hardware/manufacture/iot/add-a-registry-setting-to-an-image)。

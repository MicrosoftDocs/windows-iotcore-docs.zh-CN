...
主要功能的文档位于[Windows 设备门户](https://msdn.microsoft.com/windows/uwp/debug-test-perf/device-portal)的 "概述" 页上。 以下文档的其余部分将是特定于 IoT 的文档。
...
如果在仪表板中找不到设备或希望跳过仪表板，还可以通过输入设备的 IP 地址，并 `:8080` 到末尾来打开设备门户。 正确完成后，其外观应如下所示：
...
> [!NOTE]
> 此复选框有一个已知 bug，它会将 "flash" 从选中状态 "未选中"。 请在单击后刷新页面（F5），以确保复选框显示所需的状态。
...
这类似于动态内核转储，但适用于用户模式处理。 单击 "**下载**" 按钮将导致 "小型转储"，并将下载该进程的全部状态。 这适用于调试挂起进程。
...
单击 "**对" 链接**以将设备配对。 如果设备要求使用 PIN 进行配对，它将弹出一个显示该 PIN 的消息框。 设备配对后，它将显示在 "配对的设备" 列表中。 你可以通过单击 "**删除**" 来取消对该设备的配对。 
...
**Internet 连接共享（ICS）** Internet 连接共享允许与通过 Wi-fi SoftAP 连接到设备的其他设备共享设备的 Internet 访问。
若要使用此功能，Windows 10 IoT 设备需要有权访问 internet （例如通过有线 LAN 连接）。  在 "连接-> 载入-> SoftAP 设置" 中，单击 "启用" 并设置 SSID 名称和密码。  然后，在 "连接-> Internet 连接共享" 中为 "访问点适配器" 选择 "Microsoft Wi-fi Direct 虚拟适配器 #2" 和 "共享网络适配器"，然后选择有线以太网适配器。  最后，单击 "启动共享访问"。  启动后，将单独的 Wi-fi 启用的设备连接到 Windows 10 IoT 设备上的 SoftAP。  建立连接后，启用了单独 Wi-fi 的设备将能够通过 Windows 10 IoT 设备连接到 internet。
...
> [!NOTE]
> 接入点适配器是作为 WiFi 接入点（通常具有类似于192.168.137.1 的 IP 地址）的 WiFi 适配器。
...
受信任的平台模块（TPM）是一种加密协处理器，其中包括用于随机编号生成的功能、安全生成的加密密钥和其使用限制。 它还包括诸如远程证明和密封存储等功能。 若要了解有关 IoT Core 上的 TPM 和安全性的信息，请访问[构建安全设备](../secure-your-device/BuildingSecureDevices.md)页面和[TPM](../secure-your-device/TPM.md)页面。

> [!IMPORTANT]
> Limpet 是 Windows IoT Core 的组成部分。 从10月2018开始，它现在作为开放源代码 porject 提供[https://github.com/ms-iot/azure-dm-client](https://github.com/ms-iot/azure-dm-client)。

为了使测试更轻松，我们提供了 Limpet 的未签名预生成版本，并可直接从 WDP 下载。 只需执行 "TPM 配置" 选项卡，然后单击 "安装最新的" 按钮即可。 

> [!NOTE]
> 此版本的 Limpet 不应随最终产品一起提供。 相反，你需要构建开源项目，对其进行签名，然后将其与你的映像一起打包。

### <a name="azure-clients-configuration"></a>Azure 客户端配置

IoT 设备可以通过云服务进行远程管理。 Azure 提供了一组非常丰富的服务来实现此类方案。 我们创建了一个设备管理客户端，该客户端在 Windows 平台上补充 Azure 设备预配服务（DPS）和 Azure 的 IoT 中心服务，同时还公开了多个 Windows 可管理性功能。
...
### <a name="provisoning-device-portal-with-a-custom-ssl-certificate"></a>使用自定义 SSL 证书的预配设备门户
...
4. 验证 `HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Control\CrashControl\CrashDumpEnabled` 设置为0x1。

5. 将 `HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Control\CrashControl\DumpFileSize` 更新为0x0。
